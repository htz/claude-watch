...
- **ひと目でわかる危険度** — コマンドの危険度を自動分析し、5段階 (🟢安全 → 🟡注意 → 🟠警戒 → 🔴危険) で色分け表示。確認が必要なものだけに集中できます
- **キーボードだけで即判断** — `⌘Enter` で許可、`Esc` で拒否。マウスに手を伸ばさずコーディングの流れを維持できます
- **集中を妨げない通知** — ポップアップは今のウィンドウのフォーカスを奪いません。作業中のエディタやターミナルはそのまま使い続けられます

### Claude Code の操作をリアルタイムに把握

- **タスク完了を見逃さない** — Claude Code がタスクを終えたら即座に通知。「終わったかな？」とターミナルを確認しに行く必要がなくなります
- **入力待ちにすぐ気づく** — Claude Code があなたの応答を待っている時、手動で閉じるまで消えない通知で確実にお知らせします
- **複数リクエストも整理して表示** — 確認要求が連続しても、件数バッジ付きで一つずつ順番に処示。見落としがありません

### いつもの設定をそのまま活用

- **settings.json の権限設定に準拠** — Claude Code で設定済みの allow / deny / ask ルールをそのまま尊重。許可済みのコマンドはポップアップなしで通過します
- **`--dangerously-skip-permissions` にも対応** — 自動実行モード時は全ポップアップを自動スキップし、作業を邪魔しません

### 安心のフォールバック設計

- **アプリが落ちても Claude Code は止まらない** — Claude Watch が未起動やエラーでも、Claude Code 標準のダイアログに自動フォールバック。ワークフローが途切れることはありません
- **見逃してもスキップで継続** — ポップアップを見逃してフォーカスが外れた場合、自動的に Claude Code 側の確認に切り替わります

### その他

- **日本語 / 英語対応** — システムロケールで自動検出。トレイメニューからいつでも切り替え可能
- **ダークモード対応** — macOS のシステムテーマに自動追従
- **セキュアな通信** — Unix ドメインソケットでポート競合なし。アプリの所有者のみアクセス可能
